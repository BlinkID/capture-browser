...
import { AnalyzerSettings, FrameAnalysisResult } from "capture-wasm";
...
export const uiStateMap: Readonly<Record<UiStateKey, UiState>> = {
  ...
  // `FLIP_CARD` can only be triggered programmatically!
  FLIP_CARD: {
    key: "FLIP_CARD",
    reticleType: "flip",
    minDuration: 3000,
...
export class FeedbackParser {
  ...
  updateSettings(analyzerSettings: AnalyzerSettings) {
    this.analyzerSettings = analyzerSettings;
  }
  ...
  private getUiStateKeyFromResult(frameAnalysisResult: FrameAnalysisResult) {
    return (
      match<FrameAnalysisResult, UiStateKey>(frameAnalysisResult)
        ...
        // This state is a side-effect of "side-captured"
